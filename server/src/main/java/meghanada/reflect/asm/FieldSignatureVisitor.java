...
class FieldSignatureVisitor extends SignatureVisitor {
  ...
  public void visitTypeArgument() {
    final EntryMessage m =
        log.traceEntry(
            "mame={} typeInfo={} currentType={}", this.name, this.typeInfo, this.currentType);
    if (this.currentType.size() == 0) {
      this.visitClassType("?");
      log.traceExit();
      return;
  ...
  public void visitTypeVariable(String typeVariable) {
    final EntryMessage em =
        log.traceEntry(
            "name={} typeVariable={} typeInfo={} current={}",
            this.name,
            typeVariable,
            this.typeInfo,
            this.currentType);
    ...
    if (this.typeMap != null && typeMap.containsKey(typeVariable)) {
      ...
      if (val.equals(typeVariable)) {
        ...
        ClassNameUtils.getTypeVariable(val)
            .ifPresent(tv -> FieldSignatureVisitor.getTopVisitor(this).typeParameters.add(tv));
        typeVariable = val;
      }
    } else {
