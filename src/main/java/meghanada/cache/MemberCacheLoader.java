...
class MemberCacheLoader extends CacheLoader<String, List<MemberDescriptor>> implements RemovalListener<String, List<MemberDescriptor>> {
    ...
    private static List<MemberDescriptor> getCachedMemberDescriptors(final String fqcn,
                                                                     final File cacheFilePath,
    ...
    private static List<MemberDescriptor> getCachedMemberDescriptors(final File cacheFilePath,
                                                                     final String md5sum,
                                                                     final String filePath) throws IOException {
        final String projectRoot = System.getProperty(Project.PROJECT_ROOT_KEY);
        final File checksumMapFile = FileUtils.getProjectDataFile(new File(projectRoot), GlobalCache.COMPILE_CHECKSUM_DATA);
    ...
    public List<MemberDescriptor> load(final String className) throws IOException {
        ...
        @SuppressWarnings("unchecked") final List<MemberDescriptor> cachedResult = MemberCacheLoader.getCachedMemberDescriptors(fqcn, cacheFilePath, classFile);
        if (cachedResult != null) {
            return cachedResult;
        }
