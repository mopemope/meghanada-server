...
public class EnumScopeSerializer extends Serializer<EnumScope> {
    ...
    public void write(Kryo kryo, Output output, EnumScope scope) {
        ...
        for (final String clazz : implClasses) {
            kryo.writeClassAndObject(output, clazz);
        }

        // 15. index
        output.writeInt(scope.index, true);
    ...
    public EnumScope read(Kryo kryo, Input input, Class<EnumScope> aClass) {
        ...
        final EnumScope scope = new EnumScope(pkg, name, range, nameRange);
        ...
        // 14. implClasses
        final int implClassesSize = input.readInt(true);
        for (int i = 0; i < implClassesSize; i++) {
            final String s = (String) kryo.readClassAndObject(input);
            scope.implClasses.add(s);
        }

        // 15. index
        scope.index = input.readInt(true);
        return scope;
